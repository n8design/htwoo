 ...
 * 
 * Usage:
 *   node scripts/cross-package-sync.js [command] [options]
 * 
 * Commands:
 ...
 */
function syncReactToCore() {
    log.title('Synchronizing React to Core Version');
    
    const packages = getPackageInfo();
    const coreVersion = packages.core.version;
    const reactVersion = packages.react.version;
    
    if (coreVersion === reactVersion) {
        log.success('React is already synchronized with core');
        return true;
    }
    
    log.info(`Updating React from ${reactVersion} to ${coreVersion}`);
    
    packages.react.source.data.version = coreVersion;
    
    // Update core dependency if it exists
    if (packages.react.source.data.dependencies?.['@n8d/htwoo-core']) {
        packages.react.source.data.dependencies['@n8d/htwoo-core'] = `^${coreVersion}`;
        log.info(`Updated core dependency to ^${coreVersion}`);
    }
    
    if (writePackageJson(packages.react.source.path, packages.react.source.data)) {
        log.success('React package synchronized successfully');
        return true;
    }
...
/**
 * Update htwoo-react dependencies to match htwoo-core
 */
function syncDependencies() {
    log.title('Synchronizing Dependencies');
    
    const packages = getPackageInfo();
    
    if (!packages.react.source.data.dependencies?.['@n8d/htwoo-core']) {
        log.warning('htwoo-react does not depend on @n8d/htwoo-core');
        return true;
    }
    
    const coreVersion = packages.core.version;
    const currentDep = packages.react.source.data.dependencies['@n8d/htwoo-core'];
    const newDep = `^${coreVersion}`;
    
    if (currentDep === newDep) {
        log.success('Dependencies are already synchronized');
        return true;
    }
    
    log.info(`Updating core dependency: ${currentDep} → ${newDep}`);
    
    packages.react.source.data.dependencies['@n8d/htwoo-core'] = newDep;
...
function showHelp() {
    console.log(`
Cross-Package Version Synchronization Script

USAGE:
    node scripts/cross-package-sync.js [command] [options]

COMMANDS:
    status        Show current status of both packages
    check-sync    Check synchronization status between packages
    plan-sync     Show what would be synchronized (dry run)
    sync-both     Synchronize both packages to the same version
    sync-react    Update htwoo-react to match htwoo-core version
    sync-deps     Update htwoo-react dependencies to match htwoo-core
    bump-both     Bump both packages together (patch|minor|major)

OPTIONS:
    --version=VERSION    Target version for synchronization
    --type=TYPE         Release type for bump-both (patch|minor|major)
...
WORKFLOW:
    ...
    3. Apply synchronization: sync-both
    4. Verify: check-sync
`);
}
