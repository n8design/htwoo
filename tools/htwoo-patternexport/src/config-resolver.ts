...
import type { CompareOptions, PatternLabConfig, ResolvedConfig } from './types.js';
 ...
 */
export async function resolveConfig(options: CompareOptions): Promise<ResolvedConfig> {
    ...
    let config: PatternLabConfig | null = null;
    if (configExists) {
        try {
            const configContent = await readFile(configPath, 'utf-8');
            config = JSON.parse(configContent) as PatternLabConfig;
        } catch (error) {
 ...
 */
function resolvePatternDir(
    options: CompareOptions,
    config: PatternLabConfig | null,
    currentDir: string
): string {
 ...
 */
function resolveImagesDir(options: CompareOptions, patternsDir: string, currentDir: string): string {
