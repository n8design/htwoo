 ...
 */
async function compareFileBinary(sourcePath, exportPath, manifest, manifestKey) {
    let sourceHash = '';
    let exportHash = '';
 ...
 */
export async function compareFiles(files, config, manifest) {
    const differences = [];
    const newPatterns = [];
    ...
    const removedPatterns = detectRemovedFiles(files, manifest, config);
    return {
        differences,
        newPatterns,
        modifiedPatterns,
 ...
 */
async function compareFile(sourcePath, exportPath, manifest, manifestKey) {
 ...
 */
function detectRemovedFiles(files, manifest, config) {
    const removed = [];
 ...
 */
export async function copyChangedFiles(differences, config, manifest) {
    for (const diff of differences) {
        await copyFile(diff.sourcePath, diff.exportPath, diff.sourceHash, diff.path, diff.type, manifest, config.dryRun);
    }
}
/**
 * Copies a single file and updates manifest
 */
async function copyFile(sourcePath, exportPath, sourceHash, relativePath, fileType, manifest, isDryRun) {
    ...
    if (!isDryRun) {
        if (encoding) {
            await writeFile(exportPath, content, encoding);
        }
        else {
 ...
 */
export async function processMarkdownContent(content) {
    const yamlRegex = /^---\n([\s\S]*?)\n---\n([\s\S]*)$/;
...
//# sourceMappingURL=file-comparator.js.map